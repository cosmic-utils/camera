...
// GPU shader for Gaussian blur on RGB textures (for multi-pass blur)

@group(0) @binding(0)
var texture_rgb: texture_2d<f32>;

@group(0) @binding(1)
var sampler_rgb: sampler;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (viewport.content_fit_mode == 1u) {
        ...
        let tex_size_dim = vec2<f32>(textureDimensions(texture_rgb));
    ...
    // Get texture dimensions
    let tex_size = textureDimensions(texture_rgb);
    ...
    for (var ring = 1; ring <= 3; ring++) {
        ...
        for (var i = 0; i < samples; i++) {
            ...
            // Sample RGB texture
            let rgb = textureSample(texture_rgb, sampler_rgb, sample_coords).rgb;
    ...
    let center_rgb = textureSample(texture_rgb, sampler_rgb, tex_coords).rgb;
