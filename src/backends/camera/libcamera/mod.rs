...
//!
//! This backend uses libcamera for camera enumeration, format detection, and capture.
//! It's designed for mobile Linux devices (PinePhone, Librem 5, etc.) that use
//! the libcamera camera stack.

mod enumeration;
mod pipeline;

pub use enumeration::{enumerate_libcamera_cameras, get_libcamera_formats, is_libcamera_available};
...
/// Libcamera backend implementation
pub struct LibcameraBackend {
    ...
    pipeline: Option<pipeline::LibcameraPipeline>,
...
impl LibcameraBackend {
    ...
    fn create_pipeline(&mut self) -> BackendResult<()> {
        ...
        // Create pipeline
        let pipeline = pipeline::LibcameraPipeline::new(device, format, sender.clone())?;
...
impl CameraBackend for LibcameraBackend {
    fn enumerate_cameras(&self) -> Vec<CameraDevice> {
        info!("Using libcamera backend for camera enumeration");

        if let Some(cameras) = enumerate_libcamera_cameras() {
            info!(count = cameras.len(), "Libcamera cameras enumerated");
            cameras
        } else {
            info!("Libcamera enumeration returned None");
            Vec::new()
        }
    }

    fn get_formats(&self, device: &CameraDevice, _video_mode: bool) -> Vec<CameraFormat> {
        info!(device_path = %device.path, "Getting formats via libcamera backend");
        get_libcamera_formats(&device.path, device.metadata_path.as_deref())
    }
